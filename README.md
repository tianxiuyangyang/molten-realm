...
- 「原图对照」显示当前区域对应的参考图；参考图替换也作用于当前区域。
- 「截图」导出当前渲染画面；「全屏」扩展观察空间。
- 「精细」「超清」「流畅」三档画质用于平衡分辨率、阴影、辉光与设备性能，默认精细。

`reference.jpg` 对应原有要塞；`reference-portal.jpg` 保留新参考图的 1920 × 1079 像素尺寸。`scene-manifest.json` 保留原要塞分析，并记录两处区域坐标、新场景屏幕锚点、材质、灯光及构图假设。参考图用于对照，画布中的空间由场景几何生成。
...
已验证：桌面 1280 × 720 与窄屏 390 × 844 预览、两区域切换、鼠标环绕和参考复位、原图叠加、各区域独立替换参考图、精细/流畅画质切换、PNG 实际下载；模块语法与本地资源检查通过，未发现浏览器渲染警告或错误。新区域约 49,000 个实例构件。全屏取决于浏览器窗口是否允许原生 Fullscreen API。
